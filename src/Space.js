...
export const Space = () => {
    ...
    return (
        <div style={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", fontFamily: "IBM Plex Serif"}}>
            ...
            {isMobile ?
                <div style={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", margin: "0 1rem"}}>
                    <h1 style={{margin: "0", textAlign: "center"}}><strong>Current space: {spaceData.name} (Join code {spaceData.code})</strong></h1>
                    <h3 style={{margin: "0.5rem 0 2rem 0", fontWeight: "300", textAlign: "center"}}>[ <span onMouseEnter={() => setHoveringReturn(true)} onMouseLeave={() => setHoveringReturn(false)} style={{fontWeight: hoveringReturn ? "900" : "300"}} onClick={() => navigate("/", {state: {user: location.state.user}})}>return home</span> ]</h3>
                </div>
                :
            ...
            <h5 style={{margin: "0 1rem", fontWeight: "900", textAlign: "center"}}>Members: <span style={{fontWeight: "300"}}>{spaceData.members === [] ? "Loading..." : spaceData.members.join(", ")}</span></h5>
            { isMobile ?
                <div style={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", margin: "0 1rem"}}>
                    <h4 style={{margin: "1rem 0 0 0"}}><em>{spaceData.storyLength !== 40 ? <>Story in progress... it's {spaceData.whoseTurn}'s turn.</> : <>Story complete!</>}</em></h4>
                    <h4 style={{margin: "0.5rem 0 1rem 0"}}>[ <span style={{color: "#ffffff"}} onClick={refresh}><u>refresh</u></span> ]</h4>
                </div>
                :
            ...
            <p style={{minHeight: "10rem", width: "90%", border: "2px solid #f1f7ed", borderRadius: "1rem", padding: "1rem", margin: "1.5rem 0"}}>{spaceData.story === "" ? <em style={{opacity: "0.5"}}>This story is yet to be written...</em> : <>{spaceData.story}</>}</p>
